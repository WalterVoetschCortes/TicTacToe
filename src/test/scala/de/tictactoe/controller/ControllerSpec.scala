...
class ControllerSpec extends WordSpec with Matchers{
  ...
  "A Controller" when {
    "created" should {
      ...
      "can handle" in {
        controller.handle("player1 player2") should be("Now set the maximum score between 0 and 100")
      }
      "can set players with wrong input" in {
        controller.setPlayers("player1 player2 player3") should be("Wrong input!! Try it again!")
      }
      "can set players" in {
        controller.setPlayers("player1 player2") should be("Now set the maximum score between 0 and 100")
      }
      "can create empty game board" in {
        controller.createNewRound should be("")
      }
      ...
      "c sets O to field(0,1)" in {
        controller.set(0,1) should be ("Sorry d and c! The round ended in a tie!")
      }
    }
  }
}